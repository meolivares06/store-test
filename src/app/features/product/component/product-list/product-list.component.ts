...
@Component({
  ...
  imports: [ConfirmDialogModule, DatatableComponent, ToastModule],
...
export class ProductListComponent
  extends BaseCrudComponent<Product>
  implements OnInit
{
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.subscriptions.push(
          this.store
            .deleteFirebase(rowData)
            .pipe(
              tap(() =>
                this.messageService.add({
                  severity: 'success',
                  summary: 'Success',
                  detail: 'Executed succefully',
                }),
              ),
              catchError(() => {
                this.messageService.add({
  ...
  override onRefresh() {
    this.subscriptions.push(this.store.refresh().subscribe());
  }
